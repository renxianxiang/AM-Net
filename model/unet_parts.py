...
class DilThreefoldConv(nn.Module):

    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.ThreefoldConv = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, kernel_size=5, padding=10,dilation=5),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True),
            nn.Conv2d(out_channels, out_channels, kernel_size=5, padding=10,dilation=5),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels, out_channels, kernel_size=5, padding=10,dilation=5),
            nn.BatchNorm2d(out_channels),
    ...
    def forward(self, x):
        return self.ThreefoldConv(x)

class ThreefoldConv(nn.Module):
...
class SPPLayer(torch.nn.Module):
        ...
        def forward(self, x3, inference=False):
...
class SelfAttention(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        context_layer = context_layer.permute(0, 2, 1, 3, 4).contiguous()
...
class ConvTranspose2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.ConvTranspose2d = nn.Sequential(
...
class ConvTranspose2d3(nn.Module):
